...
> Last updated: 2026-05-18 (Phase 5 session 1 closed — Power BI Desktop connected to Snowflake under dedicated `POWERBI_READER` role; semantic model live with 4 relationships across `WAREHOUSE.fact_daily_sales` + 3 dims + `MARTS.mart_executive_overview` under the lean-marts pattern; Executive Overview page built with title, date-range slicer, 4 KPI cards (Total Revenue $93.80M / Total Units Sold 34.52M / Active Stores 10 / Active Items 2.46K peak), and a dual-axis line chart showing daily revenue + units. `sql/snowflake/04_grant_powerbi_reader.sql` ships least-privilege grants — SELECT-only on WAREHOUSE + MARTS, no access to RAW/STAGING/INTERMEDIATE. Five durable LEARNINGS entries captured: Snowflake metadata visibility ≠ access boundary (visitor-badge analogy); PBI explicit vs implicit measures (recipe-on-the-wall analogy); mart→calendar 1:1 cardinality override for star-schema discipline; PBI dual-axis charts disable trend lines; PBI Desktop UI version variance + web-check discipline rule. Backfill of the late-Jan-2014 → mid-March-2014 gap deferred to session 5.2 open — naive `airflow dags backfill` won't fix it because the fact's `is_incremental()` clause is forward-only (caught at session close); needs a `--full-refresh` or temporary code change instead, which deserves a real decision not an unattended run. Phase 5 session 2 next: backfill approach + execution, then Demand by Hierarchy + Promotion & Price pages from the warehouse star.).

---

## Where we are right now

**Current phase:** **Phase 5 session 1 ✅ DONE.** Power BI Desktop now connects to Snowflake under a dedicated **`POWERBI_READER`** role (`sql/snowflake/04_grant_powerbi_reader.sql` — SELECT-only on `WAREHOUSE.*` + `MARTS.*`, zero access to RAW/STAGING/INTERMEDIATE; principle-of-least-privilege story written down on disk, not just claimed). Semantic model live with 4 relationships under the lean-marts pattern: `FACT_DAILY_SALES → DIM_CALENDAR / DIM_ITEM / DIM_STORE` (all many-to-one, single direction) + `MART_EXECUTIVE_OVERVIEW → DIM_CALENDAR` (cardinality manually overridden from PBI's auto-detected 1:1 to many-to-one to enable Single cross-filter direction — preserves star-schema purity by keeping `dim_calendar` as the conformed filter source for both fact and mart). Autodetect-relationships and auto-date-time both disabled (Project #1 carry-forwards locked in CURRENT FILE + GLOBAL settings). **Composite storage mode** locked in after empirical evaluation: **Import** for the 4 small tables (`DIM_CALENDAR` 1,082 rows, `DIM_ITEM` 3,049, `DIM_STORE` 10, `MART_EXECUTIVE_OVERVIEW` 1,081 — VertiPaq compresses them to KB-level); **DirectQuery** for `FACT_DAILY_SALES` (32.9M rows, ~600 MB compressed in VertiPaq — stays in Snowflake and queries live). The Import-only attempt produced a 949 MB `.pbix` that exceeded GitHub's 100 MB push limit; switching the fact to DirectQuery dropped the file to 264 KB — a ~3,600× reduction — and ships the composite-mode senior-DE pattern as a real interview talk-track. **Executive Overview page** built end-to-end: page title, date-range slicer (1,082 calendar dates), 4 KPI cards (Total Revenue $93.80M, Total Units Sold 34.52M, Active Stores 10, Active Items 2.46K peak), and a dual-axis line chart showing daily revenue + units over the M5 date range — visible weekly seasonality + three Christmas-Day dips + steady growth trend + rightmost session-6 jump all coming through cleanly. All measures are explicit DAX (`Total Revenue`, `Total Units Sold`, `Active Items`, `Active Stores`) — no implicit aggregations; recipe-on-the-wall discipline. **`.pbix` saved** to `powerbi/retail_demand_forecasting.pbix`; `powerbi/.gitkeep` deleted (folder now has real content). **Five real engineering insights captured as durable LEARNINGS**: (a) **Snowflake metadata visibility ≠ access** — `SHOW SCHEMAS IN DATABASE` returns every schema in a database the role has USAGE on, regardless of per-schema privileges; USAGE controls whether you can OPEN the schema, not whether it appears in catalog listings. PBI's Navigator surfaced this verbosely; confirmed via `INFORMATION_SCHEMA.QUERY_HISTORY_BY_USER` that every query ran under POWERBI_READER and the SELECT boundary still bit on RAW. Visitor-badge analogy. (b) **PBI explicit vs implicit measures** — every visual measure created as a named DAX measure, not by dragging the raw column. Reusable, formattable centrally, base for derived measures like time intelligence in 5.5. Recipe-on-the-wall analogy. (c) **Mart→calendar 1:1 cardinality override** — when both sides of a relationship are unique, PBI locks cross-filter direction to "Both", breaking star-schema discipline if both fact and mart connect through the same dim. Fix: manually override cardinality to Many-to-one (PBI shows a benign data-integrity warning, accept it), Single direction filter then unlocks. Star-schema purity > technical accuracy. (d) **PBI dual-axis line charts disable trend lines** — when measure scales differ enough that PBI auto-converts to dual-axis, the Analytics → Trend line option is hidden. Workaround: split into two single-measure side-by-side charts. Version-independent constraint. (e) **PBI Desktop UI version variance** — Power BI Desktop ships continuous UI updates (new Card visual replaced classic Card as default Nov 2025 GA; "Recent Sources" not visible in some versions). Discipline: web-check current state of UI elements before asserting, or ask user to confirm what they see. **One real diagnostic** done mid-session: PBI's Navigator initially showed RAW/STAGING/INTERMEDIATE schemas under POWERBI_READER role; instead of guessing, ran `INFORMATION_SCHEMA.QUERY_HISTORY_BY_USER('PHELUCIAM')` which proved every PBI query genuinely used POWERBI_READER — the schema listing was Snowflake's metadata behavior, not a privilege leak. Ground-truth-first diagnostic pattern earned its keep. **One data observation flagged for queued action**: rightmost portion of the revenue line chart shows a real gap between late-Jan-2014 and 2014-03-22 (session 6's new dates), caused by sporadic Phase 3 testing rather than continuous DAG runs. Backfill deferred to session 5.2 open — caught at session close that naive `airflow dags backfill` won't fix the gap because the fact's `is_incremental()` clause is forward-only (`WHERE sale_date > MAX(sale_date)` strictly excludes historical dates). Three approaches in flight for 5.2: `dbt run --full-refresh` on the fact (heaviest but cleanest), temporary code change to a `NOT IN` predicate (surgical), or manual MERGE bypassing dbt (most surgical but least dbt-canonical). Decision deferred so it gets a real think-through rather than a 30-min unattended run. **Phase 5 session 2 next**: backfill approach + execution, then Demand by Hierarchy + Promotion & Price pages — both slice the warehouse star directly (fact + dims, not the mart), bringing the slicing-page DirectQuery-vs-Import evaluation into focus.
