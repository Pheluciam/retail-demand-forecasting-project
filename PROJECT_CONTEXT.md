...
> - **Cyclic reference errors — TWO causes, two diagnostic orders.** (a) Spurious cache desync → save + close + reopen clears instantly (5.5 pattern). (b) Real Power Query M-code self-reference → open PQ Editor, inspect each Applied Step's first argument, must be `#"PreviousStep"` not the query name itself (5.8 pattern). Try (a) first, then (b).
...
> - **PBI format pane section names vary by visual type** — Bars / Columns / Markers / Slices / Lines (web-check the visual type FIRST before recommending click paths). New Card visual field wells = Value / Categories / Tooltips / Drill through (NEVER "Fields well").
> - **PBI Desktop format pane control locations vary by variant — pin EXACT paths from screenshot before prescribing.** Matrix Row padding lives in Grid → Options. Matrix Grow to fit lives in Layout → Column width → Auto-size behavior (with Custom widths Off). Conditional formatting via Cell elements → Apply settings to → fx. Don't prescribe from docs without confirming against the user's actual pane first.
> - **Basic-license PBI Desktop variants may be missing optional new-visual features** — Reference labels on new Card visual; Page type = Drillthrough toggle on Page information (drill-through trigger may not fire even with spec-correct wiring in these variants). Feature-detect on screenshot before prescribing.
> - **PBI measure formula commits require green checkmark click** when editing.
...
> - **dim_calendar extends 60 days past max historical date** (2014-05-22). `Active Items` measure uses fact's sale_date not dim's calendar_date.
> - **When 3 things look broken at once, suspect ONE root cause** and try the cheapest single-variable fix first.
> - **PBI build order — theme + drill-through test EARLY, not at polish time.** Apply theme after first 1-2 visuals so subsequent polish layers cleanly. Wire and test drill-through with minimal source visuals before investing in source-visual formatting (failed-trigger fix often requires delete + re-add). Locked 2026-05-22 in 5.8.
> - **Categorical value renames (e.g., "actual" → "Actual") → Power Query Replace Values is the only stock-Desktop path.** In-visual rename only works for measure pills, not for category values driving column headers. Locked 2026-05-22.
> - **DAX Studio / external tools — install for all users, not per-user.** Per-user install places pbitool.json in wrong path; External Tools ribbon won't appear. Locked 2026-05-22.
> - **Pacing locked**: 1-2 steps per response, code blocks only for paste-able (DAX, file paths, command strings), plain text for everything else. PowerShell: one command per code block.
>
> `POWERBI_PLAYBOOK.md` (revised 2026-05-19, patched through 5.8) remains the locked source of truth.
