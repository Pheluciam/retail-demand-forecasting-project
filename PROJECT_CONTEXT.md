...
> Last updated: 2026-05-16 (Phase 4 session 3 closed — intermediate + warehouse layers live, 2 new models built + 1 staging test added, 20 new tests passing, GLOSSARY.md added).

---

## Where we are right now

**Current phase:** **Phase 4 session 3 ✅ DONE** — intermediate and warehouse layers both opened in one session. `dbt_utils 1.3.3` installed via `packages.yml` + `dbt deps`. Compound-key uniqueness test added to `stg_m5_sell_prices` (modern dbt 1.10+ `arguments:` syntax — deprecation caught and fixed mid-session). First intermediate model `int_sales_with_prices` built and tested (32,898,710 rows materialised as view in `RETAIL_DB.INTERMEDIATE`, 8 tests passing). First warehouse model `dim_calendar` built and tested (1,079 rows materialised as **table** in `RETAIL_DB.WAREHOUSE`, 11 tests passing, MD5 surrogate keys via `dbt_utils.generate_surrogate_key`). Live Snowsight verification covered row-count parity, row-level revenue math, NULL-price rate (34.66% — expected from M5 product lifecycle), key uniqueness, attribute eyeball, and distribution sanity (weekend rate 28.64% bang on theoretical 2/7). 10-point code-quality audit applied honestly — clean across all 10 criteria after closing two ⚠️ items in-session (anomaly check + per-model verify SQL files). Anomaly check `units_sold > 0 AND sell_price IS NULL` returned 0 rows — proves the LEFT JOIN preserved real sales, no ghost-sales data-quality issue. Two per-model verification SQL files added (`sql/verify/04_phase4_int_sales_with_prices_verification.sql`, `sql/verify/05_phase4_dim_calendar_verification.sql`) — each with numbered sections + single-row PASS/FAIL rollup, matching the Phase 1/2/3 precedent. Bonus: `GLOSSARY.md` added at project root (~155 terms across 16 sections, ~880 lines, `[Project 2]` tags on project-specific entries) — built by a parallel sub-agent during the dbt work as an uncommon-but-strong portfolio-strengthening artefact. Next session opens **Phase 4 session 4** — `dim_item`, `dim_store`, then the centrepiece `fact_daily_sales` (partitioned, incremental); `int_sales_with_prices` becomes the source for the fact, with dim FK joins layered on top.
